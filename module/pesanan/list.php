<?php
	...
	if($level == "superadmin"){
		$queryPesanan = mysqli_query($koneksi, "SELECT pesanans.*, user.nama FROM pesanans JOIN user ON pesanans.user_id=user.user_id ORDER BY pesanans.tanggal_pemesanan DESC");
	}else{
		$queryPesanan = mysqli_query($koneksi, "SELECT pesanans.*, user.nama FROM pesanans JOIN user ON pesanans.user_id=user.user_id WHERE pesanans.user_id='$user_id' ORDER BY pesanans.tanggal_pemesanan DESC");
	}
